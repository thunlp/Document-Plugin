...
def init_formatter(config, task_list, *args, **params):
    for task in task_list:
        ...
        if task == "train":
            collate_fn[task] = formatter[task].process # train_collate_fn
        elif task == "valid":
            collate_fn[task] = formatter[task].process # valid_collate_fn
        else:
            collate_fn[task] = formatter[task].process # test_collate_fn
...
def init_one_dataset(config, mode, *args, **params):
    ...
    if which in dataset_list:
        ...
        if isinstance(dataset, KaraPytorchDatasetBase):
            ...
            dataloader = DistributedDataLoader(dataset=dataset,
                                    batch_size=batch_size,
                                    shuffle=shuffle,
                                    num_workers=reader_num,
                                    collate_fn=collate_fn[mode],
...
def init_dataset(config, *args, **params):
    ...
    if config.getboolean("train", "no_valid"):
        valid_dataset = None
    else:
